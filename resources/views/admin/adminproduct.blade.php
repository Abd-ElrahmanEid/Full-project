...
@section('content')
    ...
    <form class="container" action="{{route('adminproduct.store')}}" method="post" enctype="multipart/form-data">
        @csrf
        ...
        <h5>Product Image :</h5>
        <input type="file" class="form-control" name="image" placeholder="Enter ProductImage">
        @error('image')
        <p style="color:red;">{{$message}}</p>
        @enderror
        <br>
